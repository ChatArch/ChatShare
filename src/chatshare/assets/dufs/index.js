...
Uploader.runQueue = async () => {
  ...
  if (!Uploader.auth) {
    try {
      ...
      Uploader.auth = false;
      uploader.fail(err?.message || "Login required");
      return;
    }
  }
  uploader.ajax();
...
function authRequest(method, url, options = {}) {
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    openWithCredentials(xhr, method, url, options.credentials);
...
async function doMovePath(fileUrl) {
  ...
  try {
    ...
    const res2 = await authRequest("MOVE", fileUrl, {
      headers: {
