...
以下初始化流程描述原生 Dufs。现有实例无需重新初始化；目录登录请安装 `ChatShare[server]==0.2.7`，运行 `chatshare serve`，再由运维把反向代理指向应用。登录、会话和权限由应用负责，代理仅转发。具体文件直链保持公开，目录、搜索、归档和写入需要鉴权；详见[安全与边界](security.md)。仅安装基础包不包含服务端可选依赖。
